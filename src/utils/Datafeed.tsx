...
const makeDataFeed = () => {
  ...
  return {
    ...
    async resolveSymbol(
      ...
      // const result = await getApi(`${URL_SERVER1}symbols?market=${marketInfo.address.toString()}`)
      const result1 = await getApi(`${URL_SERVER}symbols?market=${marketInfo.name}`)
    ...
    async getBars(
      ...
      try {
        const result1 = await getApi(
          // `${URL_SERVER1}history?market=${symbolInfo.market}&resolution=${resolution}&from_time=${from}&to_time=${to}`
          `${URL_SERVER}chart?name=${symbolInfo.base_name[0]}&resolution=${resolution}&from=${from}&to=${to}`
        )

        const result = result1?.data;
        // let result;
        // chartSocket &&
        // chartSocket.requestResponse({
        //   data: {type: chartType, params: { name, resolution, from, to }},
        //   metadata: '',
        // }).subscribe({
        //   onError: (error) => {
    ...
    async subscribeBars(
      ...
      while (!stopped) {
        ...
        try {
          ...
          const candle1 = await getApi(
            // `${URL_SERVER}history?market=${symbolInfo.market}&resolution=${resolutionApi}&from_time=${from}&to_time=${to}`
            `${URL_SERVER}chart?name=${symbolInfo.base_name[0]}&resolution=${resolutionApi}&from=${from}&to=${to}`
          )
          
          const candle = candle1?.data;
